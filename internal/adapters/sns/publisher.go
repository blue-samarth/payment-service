...
func NewPublisher(client PublishAPI, topicARN string, log ports.Logger) *Publisher {
	return &Publisher{client: client, topicARN: topicARN, log: log}
}

func NewPublisherFromConfig(ctx context.Context, region, topicARN string, log ports.Logger) (*Publisher, error) {
	awsCfg, err := awsconfig.LoadDefaultConfig(ctx, awsconfig.WithRegion(region))
	if err != nil {
		return nil, fmt.Errorf("sns: load aws config: %w", err)
	}
	return NewPublisher(sns.NewFromConfig(awsCfg), topicARN, log), nil
}
...
func attributes(event ports.PendingEvent) map[string]types.MessageAttributeValue {
